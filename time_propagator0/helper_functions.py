...
def eom_trm(l1, l2, t1, t2, R0, R1, R2, a, o, v, biorthonormal=False):
    ...
    val_0_l2_AT2_R1_0 = np.einsum('ijab,bj,acik,kc->', l2, R1, t2, A[o,v], optimize=True)
    val_0_l2_AT2_R1_0 -= 0.5*np.einsum('ijab,bj,acki,kc->', l2, R1, t2, A[o,v], optimize=True)
    val_0_l2_AT2_R1_0 -= 0.5*np.einsum('ijab,acij,kc,bk->', l2, t2, A[o,v], R1, optimize=True)
    val_0_l2_AT2_R1_0 -= 0.5*np.einsum('ijab,abik,kc,cj->', l2, t2, A[o,v], R1, optimize=True)
        ...
        for aa in range(nvir):
            ...
            for jj in range(nocc):
                ...
                for bb in range(nvir):
                    b = bb + 1
                    bj = nvir*(j-1) + b
                    if ai >= bj and np.abs(eta2[aa,bb,ii,jj])>1.e-8:
...
def eom_transition_moment(l1, l2, t1, t2, R0, R1, R2, a, o, v):
    ...
    val += 2 * np.einsum(
        "bi,jc,ikbd,cdjk->", R1, a[o, v], l2, t2, optimize=True
    )
    val -= np.einsum(
        "bi,jc,ck,dj,ikbd->", R1, a[o, v], t1, t1, l2, optimize=True
    )
